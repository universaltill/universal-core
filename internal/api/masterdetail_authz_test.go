...
func TestLoadMasterDetailChildren_DegradesOnErrDeniedNotOtherErrors(t *testing.T) {
	...
	children, childDefs, refLabels, degraded, err := h.loadMasterDetailChildren(ctx, deniedScope, purchaseOrderEntityDef(), purchaseOrderFormDef(), po.ID, "en")
	if err != nil {
	...
	if _, _, _, _, err := h.loadMasterDetailChildren(ctx, failScope, purchaseOrderEntityDef(), purchaseOrderFormDef(), failPO.ID, "en"); err != nil {
	...
	_, _, _, _, err = h.loadMasterDetailChildren(ctx, failScope, purchaseOrderEntityDef(), purchaseOrderFormDef(), failPO.ID, "en")
	if err == nil {
