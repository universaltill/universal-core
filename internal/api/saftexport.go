...
func (h *Handler) ownOrganizationParty(ctx context.Context, ts tenantScope) (org data.Record, ok bool, err error) {
	roleDef, err := ts.entityDef(ctx, "PartyRole")
	if err != nil {
		// ublDefLookupError, not a plain fmt.Errorf-wrapped one: this
		// helper is shared with UBL export (see doc comment above), whose
		// writeUBLError must not mistake "PartyRole has no published
		// Definition" for "the requested document doesn't exist"
		// (uc-infra#179). SAF-T's own caller never maps errors to a 404,
		// so this makes no difference on that path.
		return data.Record{}, false, ublDefLookupError{entityType: "PartyRole", err: err}
	}
	...
	if err != nil {
		...
		return data.Record{}, false, ublDefLookupError{entityType: "Party", err: err}
	}
