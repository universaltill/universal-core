-- Legacy-schema mirror of migrations/control/0002_tenant_slug.sql (the
-- same dual-track 004/0001 already follows for zitadel_org_id): the
-- shared legacy schema still backs cmd/migrate's default target and the
-- test environments CI provisions, so its tenants table needs the same
-- slug column the control-plane one gets — same DDL, same backfill.
