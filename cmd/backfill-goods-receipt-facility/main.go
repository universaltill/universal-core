...
func main() {
	...
	// this command migrates (uc-infra#160, same shape as
	// cmd/backfill-inventory-facility's fix for uc-infra#72/#123).
	...
	// falsified audit metadata this fix exists to prevent the other way
	// around — Validate() alone only rejects an EMPTY ModelVersion on an
	// agent, never a populated one on a human, so that half of the
	// mistake needs its own check.
